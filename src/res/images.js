/**
  * @author AW
  * @flow
  */
  
  const images = {
      anh: require('../assets/images/anh.jpeg'),
    bin: require('../assets/images/bin.png'),
    checkbox: require('../assets/images/checkbox.png'),
    crown: require('../assets/images/crown.png'),
    cup1: require('../assets/images/cup1.png'),
    cup2: require('../assets/images/cup2.png'),
    cup3: require('../assets/images/cup3.png'),
    down: require('../assets/images/down.png'),
    iconHome: require('../assets/images/iconHome.png'),
    iconListkey: require('../assets/images/iconListkey.png'),
    iconadd: require('../assets/images/iconadd.png'),
    icondasboad: require('../assets/images/icondasboad.png'),
    iconnotifi: require('../assets/images/iconnotifi.png'),
    image: require('../assets/images/image.png'),
    kien: require('../assets/images/kien.jpeg'),
    log: require('../assets/images/log.png'),
    logomkt1: require('../assets/images/logomkt1.png'),
    logomkt2: require('../assets/images/logomkt2.png'),
    nodata: require('../assets/images/nodata.png'),
    pen: require('../assets/images/pen.png'),
    rank: require('../assets/images/rank.png'),
    selectPhoto: require('../assets/images/selectPhoto.png'),
    takePhoto: require('../assets/images/takePhoto.png'),
    unchek: require('../assets/images/unchek.png'),
    up: require('../assets/images/up.png'),
    uploadImage: require('../assets/images/uploadImage.png'),
  }
  export default images